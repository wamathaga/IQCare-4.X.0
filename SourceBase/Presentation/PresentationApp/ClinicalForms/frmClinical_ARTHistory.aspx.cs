...
public partial class ClinicalForms_frmClinical_ARTHistory : LogPage
{
    ...
    protected void GetARTHistoryData()
    {
        ...
        if (Convert.ToInt32(theDSARTHistory.Tables[0].Rows[0]["VisitId"]) > 0)
        {
            ...
            if (Convert.ToInt32(theDSARTHistory.Tables[2].Rows[0]["PriorART"]) == 1)
            {
                ...
                Page.ClientScript.RegisterStartupScript(this.GetType(), "alert", "EnableDis('Y');", true);               
            }
            else if (Convert.ToInt32(theDSARTHistory.Tables[2].Rows[0]["PriorART"]) == 0)
            {
                rbtnknownNo.Checked = true;
                Page.ClientScript.RegisterStartupScript(this.GetType(), "alert", "EnableDis('N');", true);
            }
    ...
    protected void btnAddPriorART_Click(object sender, EventArgs e)
    {
        ...
        if ((DataTable)ViewState["GridData"] == null)
        {
            ...
            theDR["RegLastUsed"] = ""+txtLastUsed.Value+"";
        ...
        else
        {
            ...
            if (Convert.ToInt32(ViewState["UpdateFlag"]) == 1)
            {
                ...
                for(int i = 0; i < rows.Length; i ++) 
                {
                    ...
                    rows[i]["RegLastUsed"] = "" + txtLastUsed.Value + "";
                    theDT.AcceptChanges();} 
                GrdPriorART.Columns.Clear();
                BindGrid();
                Refresh();
    ...
    protected void GrdPriorART_SelectedIndexChanging(object sender, GridViewSelectEventArgs e)
    {
        ...
        if (theDT.Rows.Count > 0)
        {
            ...
            txtLastUsed.Value = String.Format("{0:dd-MMM-yyyy}", theDT.Rows[r]["RegLastUsed"]); 
    ...
    private Boolean ValidateLastUsed()
    {
        ...
        if (txtLastUsed.Value == "")
        {
            IQCareMsgBox.Show("RegLastUsed", this);
            txtLastUsed.Focus();
            return false;
        }
        if (Convert.ToDateTime(theUtil.MakeDate(txtLastUsed.Value)) > Convert.ToDateTime(theUtil.MakeDate(Application["AppCurrentDate"].ToString())))
        {
